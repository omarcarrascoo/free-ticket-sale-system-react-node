...
function TicketCard({ticketData}) {
    ...
    const fetchEvent = async () => {
      const localStorageValue = localStorage.getItem("persist:root");
        ...
        if (TOKEN != null) {
          await fetch(`http://134.199.238.36:7722/api/events/${ticketData.orderEventId}`,{
          headers: {
          ...
          await fetch(`http://134.199.238.36:7722/api/tickets/${ticketData._id}`,{
            headers: {
              'Content-Type': 'application/json',
              token: `Bearer ${TOKEN}`,
            }
    ...
    useEffect(() => {
      ...
      const handdleClick = async (e) => {
        try {
          ...
          if(TOKEN != null){
            const response = await axios.put(`http://134.199.238.36:7722/api/tickets/${ticketData._id}`,  updateData, {
              headers: {
                  'Content-Type': 'application/json',
                  token: `Bearer ${TOKEN}`
              },
  ...
  return (
    <OrderCardBox>
      <OrderCardTexts>
        ...
        {tickets.ticketAssistant?<QRCode className='qrCode' value={`http://134.199.238.36:7722/api/ticket/${tickets._id}`}/>:<OrderCardImg src={`http://134.199.238.36:7722/public/images/fakeQR.png`}/>}
        <OrderCardTexts>
            {
